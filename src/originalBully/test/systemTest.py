...
from src.originalBully.node import Node, PORT_BASE, SIM_PORT
...
def test_leader_failure_and_reelection(node_cluster):
  ...
  assert wait_until(lambda: all(n.leaderId == 2 for n in nodes if n.alive), timeout=25), "New leader election failed to stabilize."
