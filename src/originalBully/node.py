...
class Node:
    ...
    def acknowledgeElection(self, senderId):
        ...
        time.sleep(2)  # Give time for OK to be sent before starting own election
            ...
            case "ELECTION":
                if self.id > msg.senderId:
                    ...
                    if self.isLeader:
                        ...
                        self.sendMessage(
                            msg.senderId, Message("COORDINATOR", self.id, msg.senderId)
                        )
                        return
    ...
    def sendMessage(self, targetId, msg):
        try:
            ...
            print(f"Node {self.id}: Node {targetId} is unreachable.")

            # If target is leader and unreachable, start election
            if self.leaderId == targetId:
                print(f"Node {self.id} detected leader failure. Starting election.")
                self.leaderId = None
